...
- 新增操作歷程記錄系統 (ActionLogger)，支援環形緩衝區與即時控制台輸出，記錄輸入、移動跳躍、戰鬥、道具互動與系統流程等關鍵轉移點，並於全域 `window.__pixelRunLeap` 暴露日誌除錯介面與 Playwright 端對端測試。
...
- 加大觸控按鈕尺寸與按壓回饋，HUD 加上文字換行避免觸控與訊息重疊。
- 修正 P 鍵按一次翻轉兩次的問題；新增手機觸控移動、暫停流程的瀏覽器測試。
...
- 新增下方向鍵與觸控「蹲」按鈕的蹲下動作，小型、超級與火焰能力皆可使用，蹲下時停止移動並縮小碰撞框。
- 變大狀態受傷改為先掉能力（火焰→超級→小型）不扣命，並附加 2 秒無敵閃爍；小型受傷才扣命。
- 火球改為往斜下方丟出，靠重力下墜觸地彈跳前進。
- 移除 HTML 的開始遊戲按鈕，標題畫面只留 canvas 按鈕，測試改用空白鍵開始。
- 全部遊戲文字改用 2 倍解析度算圖，改善按鈕與 HUD 字體模糊。

### Fixed

- 修正碰到旗杆無法過關：過關觸發區改為覆蓋整根旗杆，旗幟小圖僅做視覺裝飾。
- 修正烏龜出生半埋且碰撞框過矮導致穿過地板：出生高度與碰撞框改為腳底對齊。
- 修正放開方向鍵後畫面滑行：攝影機跟隨平滑改為鎖定，主角本體原本即無慣性。
